...
export default props => {
    ...
    useEffect( () => {
        axios.get(`http://localhost:8000/api/incident/${props._id}`)
            .then(res => {
                setIncidentLocation(res.data.incident_location);
                setPatientName(res.data.patient_name);
                setAllergies(res.data.allergies);
                setMedications(res.data.medications);
                setMedicationsAdministered(res.data.medications_administered);
                setSystolicBP(res.data.systolic_bp);
                setDiastolicBP(res.data.diastolic_bp);
                setHeartRate(res.data.heart_rate);
                setPreviousMedicalHistory(res.data.previous_medical_history);
    ...
    const onSubmitHandler = e => {
        e.preventDefault();
        axios.put(`http://localhost:8000/api/incident/${props._id}`, {
            incident_location,
            patient_name,
            allergies,
            medications,
